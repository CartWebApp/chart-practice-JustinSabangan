...
function lineOverYears(metrics) {
  ...
  return {
    ...
    options: {
      responsive: true,
      plugins: {
        title: { display: true, text: "Sales Over Years" }
      },
      scales: {
...
function scatterReviewVsSales(publisher) {
  ...
  return {
    ...
    data: {
      datasets: [{
        label: `Review Score vs Sales (${publisher})`,
        data: points
      }]
    },
    options: {
      plugins: {
        title: { display: true, text: `Do higher reviews mean more sales? (${publisher})` }
      },
      scales: {
